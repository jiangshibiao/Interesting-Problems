...
+ 近似比为 $(1+\epsilon)OPT(I) + 1$ 的算法介绍
	...
	- 下面，**我们将考虑所有 $> \epsilon$ 的物品**(每个箱子里最多装 $\lfloor \frac{1}{\epsilon} \rfloor$ 个)。
		...
		+ 容易发现，因为每组的大小相等，$J1$ 和 $J2$ 的前后两组的个数是一样的，可以得到 $OPT(J1)$ 最多比 $OPT(J2)$ 多 $\cita = 2n\epsilon^2$ 个箱子（为了不考虑细节，这个常数 $2$ 已经放宽了，其实大概是 $1$）
		+ 因为 $OPT(I) \ge n\epsilon$，所以 $\cita \leq 2n \epsilon^2 \leq 2 \epsilon OPT(I)$
	- 再反过来，考虑所有 $\leq \epsilon$ 的物品。如果它们能全塞进原来的空隙， 
