...
+ 带权点覆盖问题的对偶解法
	- 该问题的对偶形式为：求 $\max \limits_{e \in E} y_e$，使得对于所有的 $i \in V$，$\sum \limits_{i \in e} y_e \sum w_k$ 且 $y_e \ge 0$.
	- 互补松弛条件
		+ PCS: $x_i(w_e-\sum \limits_{e \in E} y_e)=0)$
		+ DCS: $y_e(x_i+x_j-1)=0)$ 其中 $e=(i,j)$
	- 算法
		1. 初始设 $\pmb{x}=0, \pmb{y}=0$，所有边未标记。
		2. 任选一条未标记的边 $y_e$，增加 $y_e$ 的值直到某一端点 $i$ 约束变紧。
		3. $x_i=1$，把与 $i$ 相关的边都标记。
		4. 重复以上两步直到所有边都被标记。
