...
+ [线性整数规划求解](https://www.cnblogs.com/tsreaper/p/aop6.html)
	- 割平面法
	- 分枝定界法
...
+ [Vertex Cover 和 Unrelated Parallel Machine Scheduling 的 2-近似算法介绍](https://www.cnblogs.com/tsreaper/p/aop8.html)

#### bin packing 问题

+ [lzw 的 BP 总结](http://zhenweiliu.gitee.io/blog/2019/07/28/Approximation-Algorithm-3-The%20Binpacking%20Problem/)
+ FF（First Fit）算法
	- [$FF(I) \le 1.7OPT(I)+0.8$ 的证明](https://www.cnblogs.com/tsreaper/p/aop9.html)
	- 事实上这个尾巴可以拿掉，即 $FF(I) \le 1.7OPT(I)$，证明较为复杂。
	- $1.7$ 是最优近似比。
+ FFD 算法
	- FFD 算法是：先将物品按体积从大到小排序再套用 FF 做法。
	- 考虑这个例子：$ \{ 6 \times (\frac{1}{2}+\epsilon), 6 \times (\frac{1}{4}+2\epsilon), 6 \times (\frac{1}{4}+\epsilon), 12 \times (\frac{1}{4}-2\epsilon) \}$。该例子可以无限地重复，FFD 的渐进比值至少是 $\frac{11}{9}$（而且正好是这个值）
	- 该比值的最优结果是 $FFD(I) \le \frac{11}{9} + \frac{6}{9}$，做法比较复杂。
	- FFD 的绝对近似比满足：$FFD(I) < \frac{3}{2} OPT(I)$。一个有趣的拓展问题：判定一堆物品是否可以塞在两个箱子里。**$FFD$ 是该问题最优的绝对近似比算法**（不存在任何一个近似算法的绝对近似比 $< \frac{3}{2}$）。
+ 装箱问题绝对近似比的研究
...
+ 近似比为 $(1+\epsilon)OPT(I) + 1$ 的算法介绍
	...
	- 下面，**我们将考虑所有 $> \epsilon$ 的物品**(每个箱子里最多装 $\lfloor \frac{1}{\epsilon} \rfloor$ 个)。
		...
		+ 构造实例 $J1$，将每个物品的体积放大至与下一组的分界点。于是 $OPT(J1)$ 是多项式可解的，而且 $J$ 是原问题的一组可行方案。
		+ 构造实例 $J2$，将每个物品的体积缩小至与上一组的分界点。我们有 $OPT(J2) \le OPT(I) \le OPT(J1)$。
		+ 容易发现，因为每组的大小相等，$J1$ 和 $J2$ 的前后两组的个数是一样的，可以得到 $OPT(J1)$ 最多比 $OPT(J2)$ 多 $\theta = 2n\epsilon^2$ 个箱子（为了不考虑细节，这个常数 $2$ 已经放宽了，其实大概是 $1$）
		+ 因为 $OPT(I) \ge n\epsilon$，所以 $\theta \leq 2n \epsilon^2 \leq 2 \epsilon OPT(I)$
	- 再反过来，考虑所有 $\leq \epsilon$ 的物品。如果全能塞进之前的箱子，那就证毕了。否则，每个箱子的空隙必然 $< \epsilon$（除最后一个箱子），即 $J1(I) \le \frac{1}{1-\epsilon}OPT(I)+1 \le (1+\epsilon)OPT(I)+1$
...
+ 贪心算法
	- 算法描述：每当一个任务进来的时候，随便丢给一个能安排它的机器。
	- 我们将用 `Charge Scheme` 的技术证明：它的近似比正好是 $2$。
	- 对于最优解里的每个任务 $i$，考察它开始执行的时间：
		+ 若此时
+ `Best Fit` 算法
	- 算法描述：每当一个任务进来的时候，将它丢给**能安排它的机器中**（使其不超过deadline）空闲位置尽量靠后的。
	- 接下来证明，双机器时近似度至多是 $\frac{9}{5}$。
	- 对于最优解里的每个任务 $i$，考察它开始执行的时间：
