...
+ 近似比为 $(1+\epsilon)OPT(I) + 1$ 的算法介绍
	- 首先考虑 $> \epsilon$ 的物品，每个箱子里最多装 $\lfloor \frac{1}{\epsilon} \rfloor$ 个（常数个）。
	- 如果物品的种类数是常数个，我们就能多项式求解。设一共有 $k$ 种物品，我们用 $(b_1,b_2, \dots, b_k)$ 去表示每一种的个数。而且，任何一个箱子可以装的方案数是一个常数，不妨记为 $M$。装箱问题可以写成以下的整数规划问题：求解 $\min \sum \limits_{j=i}^M x_j$，使得 $\sum \limits_{j=1}^M t_{j,i}x_j \ge b_i$，其中 $x_j \in \mathbb{N^+}$
	- 
