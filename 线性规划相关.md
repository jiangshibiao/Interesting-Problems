...
+ [链接](https://www.cnblogs.com/tsreaper/p/aop6.html)
...
+ FF（First Fit）算法
	- [一个 $FF(I) \le 1.7OPT(I)+0.8$ 的证明](https://www.cnblogs.com/tsreaper/p/aop9.html)
	- 事实上这个尾巴可以拿掉，即 $FF(I) \le 1.7OPT(I)$，证明较为复杂。
	- $1.7$ 是最优近似比。
+ FFD 算法
	- FFD 问题是指：我们先将物品按体积从大到小排序，再套用 FF 做法。
	- 考虑这个例子：$ \{ \times (\frac{1}{2}+\epsilon), 6 times (\frac{1}{4}+2\epsilon), 6 \times (\frac{1}{4}+\epsilon), 12 \times (\frac{1}{4}-2\epsilon) \}$。该例子可以无限地重复，FFD 的渐进比值至少是 $\frac{11}{9}$（而且正好是这个值）
	- 该比值的最优结果是 $FFD(I) \le \frac{11}{9} + \frac{6}{9}$，做法比较复杂。
	- $FFD 的绝对近似比满足：$FFD(I) < \frac{3}{2} OPT(I)$。一个有趣的拓展：判定一堆物品是否可以塞在两个箱子里。现在已经证明，**$FFD$ 是该问题最优的绝对近似比算法**。即不存在任何一个近似算法，它的绝对近似比 $< \frac{3}{2}$。
+ 绝对近似比的研究
	- 装箱问题的渐进近似比是 $\frac{11}{9}$，已经不能被改进了。
	- 如果写成 $A(I) \le \alpha OPT(I) + \beta$ 这个形式（允许尾项），还有很多研究空间。
		+ 可以做到 $A(I) \le (1+\epsilon)OPT(I) + 1$
