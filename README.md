...
+ 一个简单有效的基于随机的 2-approximation 做法
    ...
    - 对于每个点, 有 $p$ 的概率选入集合$S$, 那么有
        $$W' = \sum \limits_{e}{w_e \cdot P[e \in cut(S, V \backslash S)]} = \sum \limits_{e} 2p(1-p)w_e \geq 2p(1-p) W $$
    - 取 $p=0.5$ 时有 $W' \geq \frac{1}{2}W$.
+ 一个更加简单的贪心做法
	- 初始化集合 $A=B=\emptyset$. 依次考虑每个点, 如果把它加入$A$集合收益大（收益指$\sum \limits_{e \in (A,B)}{w_e}$增加的量），就加入，否则加入 $B$ 集合。
	- 容易证明在这个过程中始终满足下面的性质:
	$$ \sum_{e \in (A,B)}{w_e} \geq \sum_{e \in (A,A)}{w_e} + \sum_{e \in (B,B)}{w_e} $$
	- 从而得出 $W'=\sum_{e \in (A,B)}{w_e} \geq \frac{1}{2}\sum_{e}{w_e} \geq \frac{1}{2}W$