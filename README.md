...
+ 点覆盖问题
    - $N$ 个带点权 $v_i$ 的点中选择若干个，使得每条边两端至少有一个点被选，且权值和最小。
...
+ 一个基于线性规划解的2近似做法
    ...
    - 显然有 $W^* \le W$。$W$ 是整数线性规划的最优解，也是原问题的最优解。
    - 现在我们把 $W^*$ 对应的 $\pmb{x}$ 做这样的改动：$\pmb{x'}_i=[\pmb{x}_i]$。
    - 首先注意到，$\pmb{x'}$ 也是符合该线性规划的一组解（对于任意一组限制，至少存在一个 $\pmb{x}_i \ge 0.5$，这时候对应的 $\pmb{x'}_i$ 已经取成 $1$ 了）；然后，每一个取 $1$ 的 $\pmb{x'}_i$ 的增幅最多只有 $0.5$，即新的 $W^{\'}$ 满足 $W^{\'} \le 2W^* \le 2W$。
    - 所以该做法的近似度是 $2$。
