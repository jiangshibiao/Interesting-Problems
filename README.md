...
+ 我尝试的证明：
    - 先简化问题，假设所有边权值都不相同。
        ...
        3. 在生成树里找到 $u \rightarrow v$ 的路径，依次经过了 $e_1,e_2,...e_k$ 这些边。由条件可知，$\max \limits_{i=1..k} e_i < E$ （否则就不是局部最优解了）。
    ...
    - 现在假设权值可以相同。
        1. 在之前的第 $3$ 步里，条件变成了 $\max \limits_{i=1..k} e_i \leq E$。
        2. 在之前的第 $5$ 步里，有可能会出现 $E=e_i$，此时替换边后权值不变，无法导出矛盾。
        3. 注意到，**局部最优解中存在 $e_i$ 不存在 $E$，全局最优解中存在 $E$ 不存在 $e_i$**。我们还是做一下边的替换，得到变换后的“等价局部最优解”。
        4. 经过了这样一次变换后，这个解和全局最优解之间重复的边增加了一条。基于现在这个解，我们继续进行上述的证明；每当 $e_i < E$ 时就可以导出反证法矛盾，否则就会使重复的边增加，最终会收敛到最优解。