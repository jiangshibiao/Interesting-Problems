...
+ 我尝试的证明：
    - 简化问题，假设所有边权值都不相同。
    - 将局部最优解连成一棵树，别的边都是非树边。
    - 反证，假设该局部最优解不是全局最优解。那么必然存在一条非树边 $E=(u,v)$，它在全局最优解里。
    - 在生成树里找到 $u \rightarrow v$ 的路径，依次经过了 $e_1,e_2,...e_k$ 这些边。由条件可知，$\forall i \in [1..k] e_i < E$ （否则就不是局部最优解了）。
    - 在全局最优解里删掉 $E$ 这条边后，点 $u$ 和点 $v$ 必然属于两个不同的集合了，不妨称之为 $A$ 集合和 $B$ 集合。沿着 $u \rightarrow v$ 的路径走，顺次经过的点肯定属于 $A$ 或 $B$ 中的其中一个。
    - 显然，$u \rightarrow v$ 的路径中，必然存在两个紧挨的顶点 $(p,q)$，使得 $p$ 和 $q$ 属于不同的集合。
    - 设 $e_i=(p,q)$，我们在全局最优解里删掉边 $E=(u,v)$，加入边 $e_i$，答案一定变优。产生矛盾。
