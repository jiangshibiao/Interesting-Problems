...
+ 我尝试的证明：
    ...
    2. 现在假设权值可以相同。
        ...
        2. 在之前的第 $5$ 步里，有可能会出现 $E=e_i$，此时替换的方法无法导出矛盾。
        3. 但是注意到，局部最优解中存在 $e_i$ 不存在 $E$，全局最优解中存在 $E$ 不存在 $e_i$。我们还是做一下边的替换，得到 `局部最优解'`。
        4. 经过了这样一次变换后，这个解和全局最优解之间重复的边增加了一条。于是我们可以基于 `局部最优解'`，继续进行上述的证明；每当 $e_i < E$ 时就可以导出反证法矛盾，否则就会使重复的边增加，最终会收敛到最优解。