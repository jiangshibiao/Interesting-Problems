...
+ 概率和期望
    ...
    + [构造固定概率的函数](#构造固定概率的函数)
    + [找号码牌的概率题](#找号码牌的概率题)
    + [期望抛几次硬币使得连续两次正面朝上](#抛硬币使得连续两次正面朝上)
    + [正面是反面两倍的概率](#正面是反面两倍的概率)
    + [最大化正好获得一个礼物的概率](#最大化正好获得一个礼物的概率)
+ 奇思妙想
    + [求k组出现了奇数次的数字](#求k组出现了奇数次的数字)
    + [公平分蛋糕问题](#公平分蛋糕问题)
    + [非ww串的文法表达](#非ww串的文法表达)
    + [选数问题](#选数问题)
    + [翻纸牌](#翻纸牌)
...
- 抛一枚质地均匀的硬币，一旦正面朝上次数不低于反面朝上次数的两倍就停止，求能无限进行的概率。
- 解法：
    + 转化问题：考虑一个初始在 $x=-1$ 的质点，每次有 $\frac{1}{2}$ 的概率向右移动一步，还有 $\frac{1}{2}$ 的概率向左移动两步。在 $x=0$ 处有一个吸收壁，求小球不被吸收的概率。
    + 假设小球被吸收的概率是 $p$，则 $p=\frac{1}{2}+\frac{1}{2}p^3$，解得 $p=\frac{\sqrt 5-1}{2}$。

## 最大化正好获得一个礼物的概率

- 有 $N$ 个包裹，第 $i$ 个包裹有 $p_i$ 的概率存放着礼物。现在要选择一个包裹的子集，**使得全打开后正好获得一个礼物的概率尽量的大**。
- 解法
    + 如果已经知道选择的集合，答案显然是 $\sum \limits_k p_k \prod \limits_{i \ne k} (1-p_i)$
    + 记已经打开的包裹中没有礼物的概率是 $s_0$，恰好有一个礼物的概率为 $s_1$。那么，再打开一个概率为 $p$ 的包裹会使 $(s_0, s_1) \rightarrow (s_0 (1 - p), s_1 + (s_0 - s_1) p)$。
    + 显然，当 $s_0 \leq s_1$ 时加入任何包裹都不会使答案变优。类似地，如果删除一个礼物之后 $s_0 \leq s_1$，那么删除这个包裹不会使答案变得更差。因此，任意一个最优解对应的非空包裹集合 $G$ 都满足 $s_0 \leq s_1$，且该集合每个真子集均有 $s_0 > s_1$，也即 $\forall T \subsetneqq G, \frac{s_1}{s_0} = \sum \limits_{p \in T}{\frac{p}{1 - p}} < 1$。
    ...
    + 最终的结论是，按照概率从大到小的顺序打开包裹直到 $s_0 \le s_1$ 或者全部打开为止，即可得到一个最优解。
...
+ 题意
    - 假设字符集是 $\{0,1\}$
    - 用上下文无关语法表达出所有这样的字符串 $S$：$S=xy,|x|=|y|,x \ne y$

+ 非ww串的文法表达
    - 强行表达是不行的= =。
    - 假设字符串长度为 $2n$，则至少存在一个 $i$，满足 $S_i \ne S_{n+i}$
    - 分别以 $i$ 和 $n+i$ 为中心，$i-1$ 和 $n-i$ 为半径，可以把 $S$ 分成两个中心不同、其余随意的串。
    - 我们会发现，ww串必然不存在这样的划分，但是非ww串必然存在。
    - 最终答案即为 $c=a|b, A=cAc|a, B=cBc|b, S=AB|BA$

## 选数问题

+ 有 $N(3 \times 10^5)$ 个互不相同的整数，要在其中选择至少 $\frac{1}{3}$ 个，使得任意被选的两数之和都没有被选。
+ 解法
    - 先假设这些数在值域 $[0,V]$ 之间均匀分布。选出所有出位于 $[\frac{V}{3}, \frac{2V}{3})$ 的数即可满足要求，而且它们的个数接近 $\frac{N}{3}$.
    - 如何防止被卡？每次随机一个概率 $p \in (0,1]$，然后对 $a_i$ 做一步 $b_i = [p \cdot a_i]$，对 $b_i$ 去选择。每一次期望选到的数的个数是 $\frac{N}{3}$，多随机几次即可。

## 翻纸牌

+ 题目
    - 桌面上有两堆扑克牌，$A$ 堆正面朝上的牌比 $B$ 堆多 $50$ 张。
    - 现在你被蒙住了眼睛，只能靠手去触摸和移动牌（显然你是无法摸出一张牌是否是正面朝上）。
    - 请进行一些操作，使得结束后 $A$ 堆正面朝上的牌数和 $B$ 堆相同。
+ 解法
    - 这题给人的第一感觉是：既然不能摸出正反面，为什么可以使正面朝上的牌数一致？
...
+ 题目来源于 Google Kickstart

    + 有一个环形的蛋糕，已经被切成了 $3n(n \leq 500)$ 份，每份有一个大小 $a_i$.
    + 你和 Alice、Bob 一起取蛋糕。每轮你先选择一块，然后 Alice 和 Bob 会分别取逆时针和顺时针的下一块。
    + 问你取的 $n$ 份蛋糕大小之和最大能是多少。
...
+ 解法
    ...
    + 直接贪心取是做不了的，状态很复杂。
    + 考虑设计动态规划。第一反应是设 $f_{i,j}$ 表示蛋糕 $i \sim j$ 这段的最优值，但是无法进一步转移。
    ...
    + 一个优美的结论是：只要不选紧挨着的两块，任何方案我们都能取到！
        + 可以通过构造来证明。每轮优先取空白数量最多的那一段的左侧/右侧的点，然后重复此操作。
        + 所以我们就能设计出一个简洁的 $O(N^2)$ 的动态规划了。

## 运筹学课程详细整理

[包含线性规划、近似算法等众多有趣内容](https://github.com/jiangshibiao/Course-Review/blob/master/Operations-Research/Operations-Research.md)
...
+ 证明：
    ...
    - 现在考虑权值可以相同。
        ...
        4. 经过了这样一次变换后，$T'$ 和 $T^{\ast}$ 之间重复的边增加了一条。重复上述的流程，直到导出反证法的矛盾或是推出 $T'=T^*$。
    - 注意，上述的证明过程是**有漏洞的**。**在变换了 $T$ 后，我们并没有证明 $T'$ 在它的邻域也是最优的，就不能应用局部最优解的性质。** 可以反过来进行修正：
        1. 同上述步骤，找到一条非树边 $E$。如果 $c(E)=c(e_i)$，我们固定 $T$ 不动，把 $T^\ast$ 里的边 $E$ 替换成 $e_i$。
        2. $T$ 和 $T'$ 之间重复的边增加了一条。重复上述的流程，直到导出反证法的矛盾或是推出 $T=T'$。
    - **一个牛逼的证法**
        1. 之前的证明需要一步步调整，其实可以**一步到位**。
        2. 在取 $T^\ast$ 的时候，我们不妨在所有全局最优解里，取一个 **和 $T$ 重复的边最多的解**。
        3. 如果 $T^\ast \ne T$，存在一个 $T'$ 使得 $T'$ 里 $T$ 重复的边更近，但是这和 $T^{\ast}$ 的条件矛盾。所以 $T^{\ast}=T$。

## 图的最大割问题

+ [整理自 lzw 的博客](http://zhenweiliu.gitee.io/blog/2019/10/19/Approximation-Algorithm-9-The-Maxcut-Problem/)

+ 问题描述
    - 给定边权非负的无向图 $G=(V,E)$
    - 要将其分成两个点集 $(S, V \backslash S)$
    - 最大化 $\sum_{e \in cut(S, V \backslash S)}{w_e}$
+ 一个简单有效的基于随机的 2-approximation 做法
	- 设最优解为 $W$.
    - 对于每个点, 有 $p$ 的概率选入集合 $S$, 那么有
        $$W' = \sum \limits_{e}{w_e \cdot P[e \in (S, V \backslash S)]} = \sum \limits_{e} 2p(1-p)w_e \geq 2p(1-p) W $$
    - 取 $p=\frac{1}{2}$ 时有 $W' \geq \frac{1}{2}W$.
+ 一个更加简单的贪心做法
	- 初始化集合 $A=B=\emptyset$. 依次考虑每个点, 如果把它加入 $A$ 集合收益更大就加入，否则加入 $B$ 集合。收益指 $\sum \limits_{e \in (A,B)}{w_e}$增加的量。
	- 容易证明在这个过程中始终满足下面的性质:
