...
+ 我尝试的证明：
    - 先简化问题，假设所有边权值都不相同。
        1. 将局部最优解连成一棵树，别的边都是非树边。
        2. 反证，假设该局部最优解不是全局最优解。那么必然存在一条非树边 $E=(u,v)$，它在全局最优解里。
        3. 在生成树里找到 $u \rightarrow v$ 的路径，依次经过了 $e_1,e_2,...e_k$ 这些边。由条件可知，$\max \limits_{i=1..k} v(e_i) < v(E)$ （否则就不是局部最优解了）。
        4. 在全局最优解里删掉 $E$ 这条边后，点 $u$ 和点 $v$ 必然属于两个不同的集合了，不妨称之为 $A$ 集合和 $B$ 集合。沿着 $u \rightarrow v$ 的路径走，顺次经过的点肯定属于 $A$ 或 $B$ 中的其中一个。显然，$u \rightarrow v$ 的路径中，必然存在一条边 $e_i=(p,q)$，使得 $p$ 和 $q$ 属于不同的集合。
        5. 我们在全局最优解里删掉边 $E=(u,v)$，加入边 $e_i$。全局最优解答案更变得更优，产生矛盾。
        6. 所以（任意一个）局部最优解必然是全局最优解。
    - 现在考虑权值可以相同。
        1. 在之前的第 $3$ 步里，条件变成了 $\max \limits_{i=1..k} v(e_i) \leq v(E)$。
        2. 在之前的第 $5$ 步里，有可能会出现 $v(E)=v(e_i)$，此时替换边后权值不变，无法导出矛盾。
        3. 注意到，**局部最优解中存在 $e_i$ 不存在 $E$，全局最优解中存在 $E$ 不存在 $e_i$**。我们还是做一下边的替换，得到变换后的“等价局部最优解”。
        4. 经过了这样一次变换后，这个解和全局最优解之间重复的边增加了一条。基于现在这个解，我们继续进行上述的证明；每当 $v(e_i) < v(E)$ 时就可以导出反证法矛盾，否则重合的边会一直增加，直到收敛到最优解。
    - 注意，上述的证明过程是有漏洞的。**在变换了局部最优解后，我们还没有证明新的解在它的邻域也是最优的，就不能应用局部最优解的性质。** 下面考虑反过来构造：
        1. 同上述步骤，找到一条非树边 $E$。如果 $v(E)=v(e_i)$，我们固定局部最优解不动，而是把全局最优解里的边 $E$ 替换成 $e_i$。
        2. 经过了这个步骤，全局最优解和局部最优解重合的边也是增加了一条。重复这样的操作，直到找到 $v(e_i) < v(E)$ 并用反证法证明，或是等全局最优解一步一步收敛到局部最优解。
