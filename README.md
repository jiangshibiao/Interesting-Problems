...
+ 点覆盖问题
    - $N$ 个带点权的点中选择若干个，使得每条边两端至少有一个点被选，且选的点权值和 $v_i$ 最小。

+ 一个基于线性规划解的2-近似做法
    - 设 $x_i$ 表示第 $i$ 个点是否选（$0$/$1$），我们可以列出一个整数线性规划问题；每条边 $(u,v)$ 即为一个限制 $x_u+x_v \ge 1$，目标函数即为 $W=\sum v_i x_i$.
    - 我们先将这个线性规划一般化， $0 \le x_i \le 1$。这样能在多项式内求出一个最优解 $W^*$。
    - 显然有 $W^* \le W$。其中 $W$ 表示整数线性规划的最优解（所以也是原问题的最优解）。
    - 现在我们把 $W^*$ 对应的 $\pmb{x}$ 做这样的改动：$\pmb{x'}_i=[\pmb{x}_i]$（四舍五入）。
    - 首先注意到，$\pmb{x'}$ 也是符合该线性规划的一组解（对于任意一组限制，至少存在一个 $\pmb{x}_i \ge 0.5$，这时候对应的 $\pmb{x'}_i$ 取成 $1$，满足限制要求）；然后，每一个取 $1$ 的 $\pmb{x'}_i$ 的增幅最多只有 $0.5$，即新的 $W^{\'}$ 满足 $W^{\'} \le 2W^* \le 2W$。
    - 所以该做法的近似度是 $2$。
...
+ 文法表达
    - 假设字符集只有 $0$ 和 $1$。
    - 用文法表达出所有这样的字符串 $S$：$S=xy,|x|=|y|,x \ne y$
    - 文法是指，在已知字符和字符串之间，利用“连接”、“条件或”这两个运算符来形式化地表达一个串。
    - 比如 $S=0S1|1S0|0S0|1S1|\epsilon$ 表示所有偶数长度的串。
