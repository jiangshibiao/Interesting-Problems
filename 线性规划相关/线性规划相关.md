...
[Google浏览器 MathJax 插件安装](https://chrome.google.com/webstore/detail/tex-all-the-things/cbimabofgmfdkicghcadidpemeenbffn)（可以把本文的 Latex 公式渲染出来）

## 凸集、凸函数、线性规划的定义
+ **凸集**：若对于集合 $S$，任取 $x, y \in S$ 和 $\forall \theta \in [0,1]$ 都满足 $\theta x + (1-\theta)y \in S$，则称集合 $S$ 是凸集（Convex Set）。
  	- 凸集的交仍然是凸集。
+ **凸函数**：对于定义在凸集 $S$ 上的函数 $f(x)$，若对于 $\forall \theta \in [0,1]$ 有 $f(\theta x + (1-\theta) y) \le \theta f(x) + (1-\theta) f(y)$，那么称 $f(x)$ 是凸函数（Convex Function）。
  	- 推广：**延森不等式**（Jensen's inequality）：若 $f(x)$ 是凸函数，那么对于任意 $\sum_{i=1}^n \theta_i = 1$，有 $f(\sum_{i=1}^n \theta_ix_i) \le \sum_{i=1}^n \theta_i f(x_i)$。
+ **线性规划**：如下形式的问题被称为线性规划（Linear Programming）。  
		![](LP.png)
