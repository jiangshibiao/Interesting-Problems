...
## 凸集、凸函数、线性规划的定义
+ **凸集**：若任取 $x, y \in S$ 和 $\forall \theta \in [0,1]$ 都满足 $\theta x + (1-\theta)y \in S$，则称集合 $S$ 是**凸集（Convex Set）**。
  	- 凸集的交仍然是凸集。
...
+ **极点**：设 $S$ 为凸集，若 $x \in S$ 无法表示为其它两个 $S$ 内元素的凸组合，那么 $x$ 是**极点（extreme point）**。
...
+ **基可行解**：我们讨论 $Ax = b$ 有解且行满秩的情况（如果不满秩就去掉线性相关的限制条件）。设 $A$ 是一个 $m \times n$ 的矩阵，根据线性代数的知识，我们可以从 $A$ 中选出最多 $m$ 列线性无关的列向量，其它列向量都和它们线性相关。我们把这 $m$ 个列向量调整到前面去，把 $A$ 分成两部分：$A = \begin{bmatrix} A_B & A_N \end{bmatrix}$.容易构造出 $Ax = b$ 的一个解：$x = \begin{bmatrix} A_B^{-1}b \quad 0 \end{bmatrix}^T$，称这种解为**基可行解（Basic Feasible Solution）**。显然，基可行解至多有 $C_n^m$ 种。
    ...
    - 定理2：**最优解一定可以在基可行解处取到**。
+ [参考资料](https://www.cnblogs.com/tsreaper/p/aop1.html)
...
+ **单纯形表（Simplex Tabuleau）**
	- 把变量分成基变量和非基变量两部分：设 $c^T = \begin{bmatrix} c_B^T & c_N^T \end{bmatrix}$， $A = \begin{bmatrix} A_B & A_N \end{bmatrix}$，$x = \begin{bmatrix} x_B^T & x_N^T \end{bmatrix}^T$
...
+ **两阶段法（Two-Phase Method）**
	- 添加了 $\bar{x}$ 后，我们考虑一个新的线性规划：约束不变，优化 $\min {\sum \bar{x}_i}$ 这个函数，初始可行解是 $x = \pmb{0}, \bar{x} = b$.
	- 如果这个优化问题的最优解的目标函数值不为 $0$，那么原问题无可行解，否则我们就找到了原问题的一组可行解。我们再以这个可行解为起点，利用单纯形法求出原问题的最优解即可。
...
+ [参考资料](https://www.cnblogs.com/tsreaper/p/aop3.html)
...
+ 对偶定理
	...
	- **互补松弛定理 (Complementary Slackness)：若 $x^\ast$ 与 $y^\ast$ 分别是原问题和对偶问题的可行解，那么以下两点等价**：
...
+ 对偶单纯形法步骤（以 $\min$ 模型为例）
	...
	4. 重复步骤 $2$。
+ [参考资料](https://www.cnblogs.com/tsreaper/p/aop4.html)

## 原始对偶

+ 基本思想：
	- 互补松弛条件是 $(y^{\ast {\rm T}}A-c^{\rm T})x^\ast=0$ 且 $y^{\ast {\rm T}}(Ax^\ast-b) = 0$，其中 $x^\ast$ 和 $y^\ast$ 是原问题和对偶问题的可行解。
...
+ 经典例子：$s$ 到 $t$ 的最短路问题
	...
	- 值得注意的是，原问题的对偶问题（$\forall e=(u \rightarrow v,w) ~~~ \pmb{x}_v  \le \pmb{x}_u + w$）也具有可解释性。下面将直接证明：原问题的对偶问题计算出来的结果就是最短路的长度。
		...
		6. 最终我们得到 $F=G$，原命题得证。
+ [参考资料](https://www.cnblogs.com/tsreaper/p/aop5.html)
...
+ 01背包
	- 背包问题的近似做法
      	+ 朴反向背包算法：设 $f_{j,i}$ 表示前 $j$ 个物品中，选的价值总和恰好是 $i$，最少需要的空间。直接做的复杂度是 $O(n^2V_{max})$，能得到最优解。
        + 现在对价值进行 Scaling：把每个物品的价值调整成 $\lfloor \frac{V_i}{k} \rfloor$后再DP，$k$ 是某个常数。
    - 下面证明：该做法是**完全多项式算法**（既是读入规模的多项式，也是近似比$\epsilon$ 的多项式）。
...
+ **Gomory 割平面法**
	...
	- 每当我们用单纯形法求解后获得的不全是整数解，选择一个**非整数的变量 $x_i$**，其必然满足：
...
- **分枝定界法**
	...
	- 我们先解实数线性规划，得到了整数规划最优解的**上界**。假设最优解中 $x_i(k < x_i < k+1)$ 不是整数，就会有两种可能：$x_i \le k$ 或 $xi \ge N+1$，对两种情况分别进行搜索。
	- 如果在某一枝内算出了一个整数解，我们就得到了原整数规划最优解的**下界**。
	- 某个搜索分支结束有三种可能：
		+ 当前线性规划无可行解（*死枝*）。
		+ 当前（实数）最优解未超过下界（*枯枝*）。
		+ 当前已经是整数解（*活枝*）。
- [参考资料](https://www.cnblogs.com/tsreaper/p/aop6.html)

## 独立系统

+ [独立系统介绍](https://www.cnblogs.com/tsreaper/p/aop7.html)
+ 独立系统的交：两个独立集系统 $(E,\mathcal F_1)$ 和 $(E, \mathcal F_2)$ 的交即为 $(E, \mathcal F_1 \cup \mathcal F_2)$.
...
+ 近似比为 $(1+\epsilon)OPT(I) + 1$ 的算法介绍
	...
	- 下面，**我们将考虑所有 $> \epsilon$ 的物品**(每个箱子里最多装 $\lfloor \frac{1}{\epsilon} \rfloor$ 个)。
		...
		+ 因为 $OPT(I) \ge n\epsilon$，所以 $\theta \leq n \epsilon^2 \leq \epsilon OPT(I)$
...
+ 作业题：设计一个绝对近似比是 $1.5$ 的线性装箱算法
	...
	- 如果总共用了 $K$ 个箱子，显然 $K \le OPT$（因为是密密地叠着的）。但是现在不一定是可行解。
	- **被切开的物品最多只有 $K-1$ 个。**我们把它们从箱子中抽出来，两两组合装箱，用到了 $\lceil \frac{K-1}{2} \rceil$ 个箱子。
	- 即 $NOW \le 1.5 OPT$
...
+ [更多TSP问题的探讨](https://www.cnblogs.com/tsreaper/p/aop11.html)
...
+ 带权点覆盖问题的LP解法
	...
	- 然后，每一个取 $1$ 的 $x'_i$ 增幅最多只有 $0.5$，即新的 $W^'$ 满足 $W^' \le 2W^*$。
    - 所以该做法的近似度是 $2$。

+ 带权点覆盖问题的对偶解法
	- 上述做法求解时需要线性规划。接下来这种 2-近似 做法只需用到原始对偶的思想，可以避开线性规划求解。
	...
	- 首先我们证明：若该条件成立，近似比为 $2$（$x^{\ast}$ 表示原问题的最优解）。
		+ $\sum \limits_{i=1}^n (\sum \limits_{i \in e} y_e)x_i = \sum \limits_{e \in E} y_e(x_i+x_j) \le 2\sum \limits_{e \in E}y_e \le 2\sum \limits_{i=1}^n w_ix^{\ast}_i \le 2OPT$
	- 算法
...
+ [UPMS问题](https://www.cnblogs.com/tsreaper/p/aop8.html)
